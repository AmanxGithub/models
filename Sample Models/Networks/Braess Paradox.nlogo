...
to determine-congestion ; patch procedure
  ifelse last-here = 0 [
...
The commuters in this model will come into existence at the starting node of the traffic network in the upper left corner. The commuters are represented graphically as car or truck, but this is just for visual effect. All commuters adhere to the same behavioral rules. Some heuristic or algorithm is used by the commuters to determine the path they will take through the network before it begins. The commuter then moves through the model, one road square (patch) at a time and remains in that patch for a number of ticks before moving on to the next patch. When the commuter reaches the ending node it reports the path it took through the network and also the total number of ticks it spent in transit. This information is saved by the model and used to generate a "traffic report" which new commuters can then incorporate into their algorithm or heuristic to choose the route they take.
...
The __spawn-rate__ slider determines how often new vehicles appear at the starting node. The road congestion is determined in proportion to the __spawn-rate__ however, so __spawn-rate__ will not affect travel time at all.

The __middle-on?__ switch determines whether or not cars can pass down the middle road. If the __middle-on?__ switch is turned off all vehicles currently taking the middle road will finish their trip before the middle road is disabled.
...
To plot histograms of the different probabilities of routes being taken and the current routes that turtles are on, we use __plotxy__ because there is no way to naturally histogram across different vairables.
